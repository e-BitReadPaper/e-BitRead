...
class Client:
    def __init__(self, type, traceDir="", client_id=None):
        self.bandwidthList = []
        ...
        if self.type == "test":
            ...
            self.csvFileName = f"{traceDir}/client_{self.client_id}_{dt}.csv"
            self.csvFile = open(self.csvFileName, 'w')
            self.csvFile.write("No\tcSize\tHit\tbuffer\tbI\taBI\tlastBI\tbitrate\tthroughput\t"
                             "hThroughput\tmThroughput\tdownloadT\trebufferT\tqoe\treward\ttime\tbusy\n")


    def getBandwidthList(self, fileName):
        """获取带宽列表
        Args:
            fileName: 可以是单个文件路径或文件路径列表
        Returns:
            带宽列表
        """
